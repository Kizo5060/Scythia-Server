...
public class Bank extends ItemContainer {
	...
	private static String getTotalValue(Player player, boolean selectTab) {
		int totalValue = 0;

		for (int i = 0; i < player.getBanks().length; i++) {
			if (selectTab) {
				if (player.getCurrentBankTab() != i && player.getCurrentBankTab() != 0)
					continue;
			}
			Bank bank = player.getBank(i);
			for (Item item : bank.getValidItems()) {
				totalValue += item.getDefinition().getValue() * item.getAmount();
				if (totalValue >= Integer.MAX_VALUE || totalValue < 0)
					return "Value Exceeds Measure!";
			}
		}
		return "@gre@" + Misc.insertCommasToNumber("" + totalValue + "") + "GP";
	}

	public static void sendBankTitle(Player player) {
		player.getPacketSender().sendString(5383, "@whi@Bank of NexArch || Value: " + Bank.getTotalValue(player, true));
	}
	...
	public Bank open(Player player, boolean reset) {
		...
		if (player.getLocation() == Location.GODS_RAID || player.getLocation() == Location.MELEE_BATTLE_ROYALE) {
			return this;
		}
		...
		player.getPacketSender().sendConfig(115, player.withdrawAsNote() ? 1 : 0)
				...
				.sendConfig(111, player.isPlaceholders() ? 0 : 1).sendInterfaceSet(5292, 5063);

		Bank.sendBankTitle(this.getPlayer());
		return this;
	}
		...
		for (Item it : from.getValidItems()) {
			if (p.getGameMode() == GameMode.HARDCORE_IRONMAN) {
				if (p.getBank(p.getCurrentBankTab()).getFreeSlots() <= 322
					...
					/*
					 * Item toDrop = new Item(ItemDefinition.forId(it.getId()).isNoted() ?
	...
	public static class BankSearchAttributes {
		...
		public static void stopSearch(Player player, boolean openBank) {
			...
			player.getPacketSender().sendString(27002, Integer.toString(0)).sendString(27000, "1").sendConfig(117, 0)
					.sendString(5383, "        The Bank of Zanaris-Ps");
			if (openBank)
