...
public final class WeaponAnimations
{
	...
	public static CharacterAnimations getUpdateAnimation(Item item) {
		...
		else if (weaponName.contains("dharok") || weaponName.contains("bludgeon") || weaponName.equalsIgnoreCase("lightning hammer")) {
		...
		else if (weaponName.contains("tesla")) {
			playerStandIndex = 1461;
			playerWalkIndex = 1468;
			playerRunIndex = 1467;
		}
	...
	public static int getAttackAnimation(Player c) {
		...
		if(weaponName.equals("staff of light") || weaponId == 21005) {
	...
	/** ADD GFX ON VICTIM FOR MELEE **/
	public static int getAttackGraphics(Player c) {
		int weaponId = c.getEquipment().getItems()[Equipment.WEAPON_SLOT].getId();
		switch(weaponId) {
