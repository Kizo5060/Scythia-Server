...
public class ButtonClickPacketListener implements PacketListener {
	...
	public static final boolean isBlockedGameMode(Player player) {
		GameMode[] mode = { GameMode.IRONMAN, GameMode.HARDCORE_IRONMAN };
	...
	public void handleMessage(Player player, Packet packet) {
		...
		if (player.getFuserHandler().button(id) || player.getFuserHandler().selectTab(id)) {
			return;
		}
		...
		case 22226:
			if (!player.isApplyDr()) {
				player.setApplyDr(true);
				player.sendMessage("<shad=1>@whi@You Now Viewing Drop Rate With Bonus DR @gre@ON");
			} else {
				player.setApplyDr(false);
				player.sendMessage("<shad=1>@whi@You Now Viewing Drop Rate With Bonus DR @red@OFF");
			}
			if (player.def != null)
				DropTableInterface.getInstance().select(player, player.def);
			else
				player.sendMessage("Npc Has No Drops");
			break;
		...
		case -19428:
			player.getFuserHandler().upgrade();
			break;

		case -17500:
			if (!player.getClickDelay().elapsed(5000)) {
				player.sendMessage(
						player.getUsername() + "@red@ Please wait a 5 secounds before trying to claim a reward again.");
				return;
			}
			if (player.getInventory().contains(18689) && player.getInteractingObject().getId() == 5273) {
				...
				player.getChestViewer().open(18689, common, uncommon, rare);
				player.getClickDelay().reset();
				player.getPacketSender().sendInterfaceRemoval();
				World.sendMessage("<img=10>@blu@" + player.getUsername() + " has just opened an Owner Chest!");
				return;
			} else {
				player.getPacketSender().sendMessage(player.getUsername()
						+ ", Sorry you require the Key. Please contact staff to purchase this product.");
			}
			/*
			 * if (player.getInventory().contains(19645)) { int[] common = new int[]{2572,
			 * 11005, 14460, 14462, 18896, 18898, 18900}; int[] uncommon = new int[]{10503,
			 * 11654, 20522, 11659, 11653, 20146, 11650}; int[] rare = new int[]{10682,
			 * 10683, 10684, 10685, 940, 11039, 11040, 11041, 11042, 10480, 19888};
			 * player.getChestViewer().open(6759, common, uncommon, rare);
			 * player.getClickDelay().reset();
			 * player.getPacketSender().sendInterfaceRemoval();
			 * player.getInventory().delete(19645); World.sendMessage("<img=10>@blu@" +
			 * player.getUsername() + " has just donated opened the Donor Chest #2");
			 * player.moveTo(GameSettings.DONOR_CHEST_ZONE.copy()); return; } else {
			 * player.getPacketSender() .sendMessage(player.getUsername() +
			 * ", Sorry you require the Key. Please contact staff to purchase this product."
			 * );
			 * 
			 * } if (player.getInventory().contains(18665)) { int[] common = new
			 * int[]{20100, 20102, 11896, 20104, 10822, 10824, 10826, 11206, 11208, 7082};
			 * int[] uncommon = new int[]{11896, 10502, 20126, 20136, 20138, 20140, 20142,
			 * 20144, 20656}; int[] rare = new int[]{11661, 11662, 11679, 11680, 11681,
			 * 11682, 11658, 20520, 20658, 20659, 20660}; player.getChestViewer().open(6759,
			 * common, uncommon, rare); player.getClickDelay().reset();
			 * player.getPacketSender().sendInterfaceRemoval();
			 * player.getInventory().delete(18665); World.sendMessage("<img=10>@blu@" +
			 * player.getUsername() +
			 * " has just donated X Amount and opened the Donor Chest #3");
			 * player.moveTo(GameSettings.DONOR_CHEST_ZONE.copy()); return; } else {
			 * player.getPacketSender() .sendMessage(player.getUsername() +
			 * ", Sorry you require the Key. Please contact staff to purchase this product."
			 * ); } if (player.getInventory().contains(18647)) { int[] common = new
			 * int[]{20650, 20651, 20652, 20653, 20654, 937, 20658, 20659, 20660}; int[]
			 * uncommon = new int[]{996, 965, 5085, 5086, 5087, 5088, 5089, 18876, 11651,
			 * 5082, 5083, 5084}; int[] rare = new int[]{799, 894, 895, 896, 798, 938, 8860,
			 * 8861, 8862, 8871, 8656, 8657, 8658, 8659, 8660,};
			 * player.getChestViewer().open(6759, common, uncommon, rare);
			 * player.getClickDelay().reset();
			 * player.getPacketSender().sendInterfaceRemoval();
			 * player.getInventory().delete(18647); World.sendMessage("<img=10>@blu@" +
			 * player.getUsername() +
			 * " has just donated X Amount and opened the Donor Chest #4");
			 * player.moveTo(GameSettings.DONOR_CHEST_ZONE.copy()); return; } else {
			 * player.getPacketSender() .sendMessage(player.getUsername() +
			 * ", Sorry you require the Key. Please contact staff to purchase this product."
			 * ); }
			 */
			if (player.getInventory().contains(14471) && player.getInteractingObject().getId() == 5279) {
				...
				player.getChestViewer().open(14471, common, uncommon, rare);
				player.getClickDelay().reset();
				player.getPacketSender().sendInterfaceRemoval();
				World.sendMessage("<img=10>@gre@" + player.getUsername() + " has just opened a Slayer Chest!");
				return;
			} else {
				player.getPacketSender().sendMessage(player.getUsername()
						+ ", Sorry you require the Key. Please contact staff to purchase this product.");
		...
		case -17497:
			int ownerKey = 18689; // Owner Key
			int slayerKey = 14471; // Slayer Key

			int ownerKeyAmount = player.getInventory().getAmount(ownerKey);
			int slayerKeyAmount = player.getInventory().getAmount(slayerKey);

			int ownerObjId = 5273;
			int slayerObjId = 5279;

			if (player.getInteractingObject().getId() == ownerObjId && ownerKeyAmount > 0) {
				int[] common1 = new int[] { 18338, 915, 18057, 10942, 18058, 18689, 18958, 8655, 8654, 10902,
						10900, 10901, 2746, 21043, 12601, 3499, 3502, 6855, 6856, 12502 };
				int[] uncommon1 = new int[] { 13016, 6199, 10934, 19111, 18060, 13035, 13034, 8676, 8668, 3523, 19077, 2732,
						10867, 10869, 19908, 19909, 6853, 12279, 12278 };
				int[] rare1 = new int[] { 21056, 21055, 19002, 19003, 18061, 18960, 2996, 18063, 10935, 1413, 2786, 2805,
						2804, 2800, 19039, 3525, 10537, 2746, 13034};

				if (!player.getClickDelay().elapsed(5000)) {
					player.sendMessage(player.getUsername()
							+ "@red@ Please wait 5 seconds before trying to claim a reward again.");
					return;
				}

				player.getClickDelay().reset(); // Reset delay for the whole set of keys being opened
				// for (int i = 0; i < ownerKeyAmount; i++) {
				player.getChestViewer().openAll(ownerKey, common1, uncommon1, rare1);
				// player.getInventory().delete(ownerKey, 1); // Remove one key at a time
				// }
				World.sendMessage("<img=10>@blu@" + player.getUsername() + " has just opened " + ownerKeyAmount
						+ " Owner Chest(s)!");
				player.getPacketSender().sendInterfaceRemoval();
			} else if (slayerKeyAmount > 0 && player.getInteractingObject().getId() == slayerObjId) {
				int[] common2 = new int[] { 19087, 19088, 19089, 19090, 19091, 19092, 6830, 2709, 15492, 19092, 915,
						935, 1053 };
				int[] uncommon2 = new int[] { 14471, 15246, 9850, 7960, 6832, 2856, 2857, 17662, 17666, 2769, 2753, 3491,
						18057 };
				int[] rare2 = new int[] { 6828, 2858, 2869, 2859, 13024, 13025, 3526, 3525, 13327, 1002, 13029, 8467, 8465, 18058 };
				...
				player.getClickDelay().reset(); // Reset delay for the whole set of keys being opened
				// for (int i = 0; i < slayerKeyAmount; i++) {
				player.getChestViewer().openAll(slayerKey, common2, uncommon2, rare2);
				// player.getInventory().delete(slayerKey, 1); // Remove one key at a time
				// }
				World.sendMessage("<img=10>@gre@" + player.getUsername() + " has just opened " + slayerKeyAmount
						+ " Slayer Chest(s)!");
				player.getPacketSender().sendInterfaceRemoval();
			} else {
				player.getPacketSender().sendMessage("You don't have any keys to use.");
			}
			return;
		/* Mystery box */
		case -18532:
		...
		case -28734:
			...
			break;
		case -28033:
			if (BalloonDropParty.getDonation() >= 50) {
				BalloonDropParty.spawn(player);
				World.sendMessage("<col=FFAABC><shad=1> Drop Party has started, do ::dropparty to join!");
			} else
				player.sendMessage("We need 50$ donated to start Drop Party");
			break;
		case -27933:
			player.getPacketSender().sendInterfaceRemoval();
			break;
		...
		case -7533:
			if (player.getLocation() == Location.RAIDS_ONE_ENTRANCE
					...
					|| player.getLocation() == Location.RAIDS_EIGHT_ENTRANCE
					|| player.getLocation() == Location.RAIDS_DD_ENTRANCE
					|| player.getLocation() == Location.RAIDS_DS_ENTRANCE

			) {
				if (player.getMinigameAttributes().getRaidsAttributes().getParty() != null) {
					if (player.getMinigameAttributes().getRaidsAttributes().getParty().getOwner() != player) {
						player.getPacketSender().sendMessage("Only the party leader can attemptInvite other players.");
					} else {
		...
		case -7530:
			if (player.getLocation() == Location.RAIDS_ONE_ENTRANCE
					...
					|| player.getLocation() == Location.RAIDS_EIGHT_ENTRANCE
					|| player.getLocation() == Location.RAIDS_EIGHT_PHASE_ONE
					|| player.getLocation() == Location.RAIDS_DD_ENTRANCE
					|| player.getLocation() == Location.RAIDS_DD_PHASE_ONE
					|| player.getLocation() == Location.RAIDS_DD_PHASE_TWO
					|| player.getLocation() == Location.RAIDS_DD_PHASE_THREE
					|| player.getLocation() == Location.RAIDS_DS_ENTRANCE
					|| player.getLocation() == Location.RAIDS_DS_PHASE_ONE
				...
				if (player.getLocation() == Location.RAIDS_ONE_ENTRANCE) {
					...
					player.moveTo(new Position(3056, 2840, 0));
				} else if (player.getLocation() == Location.RAIDS_FOUR_ENTRANCE) {
					player.moveTo(new Position(2846, 3102, 0));
				} else if (player.getLocation() == Location.RAIDS_FIVE_ENTRANCE) {
					player.moveTo(new Position(2909, 2516, 0));
				} else if (player.getLocation() == Location.RAIDS_SIX_ENTRANCE) {
					player.moveTo(new Position(3097, 3939, 0));
				} else if (player.getLocation() == Location.RAIDS_SEVEN_ENTRANCE) {
					player.moveTo(new Position(1818, 3099, 2));
				} else if (player.getLocation() == Location.RAIDS_THREE_ENTRANCE) {
					player.moveTo(new Position(1246, 1247, 0));
				} else if (player.getLocation() == Location.RAIDS_EIGHT_ENTRANCE) {
					player.moveTo(new Position(3285, 4000, 0));
				} else if (player.getLocation() == Location.RAIDS_DD_ENTRANCE) {
					player.moveTo(GameSettings.RAIDS_DD_LOBBY);
				} else if (player.getLocation() == Location.RAIDS_DS_ENTRANCE)
					player.moveTo(GameSettings.RAIDS_DS_LOBBY);
				// player.getPacketSender().sendTabInterface(GameSettings.ACHIEVEMENT_TAB,
				// 58000);
		...
		case 28180:
			...
			player.getPacketSender().sendMessage("<shad=0>@yel@Dungeoneering Is Currenty Disabled. Use lamps at Afk Shop to level this skill.");
		...
		case -26373:
			if (WellOfGoodwill.isActive()) {
				player.getPacketSender().sendMessage(
						"<img=10> <col=008FB2>The Well of Goodwill is granting Bonuses experience for another "
		...
		case 11014:
			...
			player.moveTo(new Position(2967, 3212), true);
			break;
		case -26333:
			player.getPacketSender().sendString(1, "https://nexarch.teamgames.io/services/store");
			player.getPacketSender().sendMessage("Attempting to open: NexArch HomePage");
			break;
		...
		case 8658:
			...
			player.getPacketSender().sendMessage("<shad=0>@yel@Agility Is Currenty Disabled. Use lamps at Afk Shop to level this skill.");
		...
		case 8672:
			...
			break;
		case 8861:
			TeleportHandler.teleportPlayer(player, new Position(3020, 2785), player.getSpellbook().getTeleportType());
			break;
		case 8656:
			...
			break;
		case 8659:
			TeleportHandler.teleportPlayer(player, new Position(2035, 4643), player.getSpellbook().getTeleportType());
			break;
		case 8664:
			TeleportHandler.teleportPlayer(player, new Position(2035, 4643), player.getSpellbook().getTeleportType());
			break;
		case 8666:
			TeleportHandler.teleportPlayer(player, new Position(2961, 3212), player.getSpellbook().getTeleportType());
			break;
		...
		case 8668:
			...
			break;
		case 8665:
			TeleportHandler.teleportPlayer(player, new Position(2008, 4635), player.getSpellbook().getTeleportType());
			break;
		case 8662:
		...
		case 13928:
			...
			break;
		case 28179:
			TeleportHandler.teleportPlayer(player, new Position(2022, 4640), player.getSpellbook().getTeleportType());
			break;
		case 28178:
		...
		case 11004:
			...
			break;
		/*
		 * case 8654: case 8657: case 8655: case 8663: case 8669: case 8660: case 11008:
		 * Teleporting.openTab(player, -4934); break;* case 11017:
		 * Teleporting.openTab(player, -4931); break; case 11011:
		 * Teleporting.openTab(player, -4919); break; case 11020:
		 * Teleporting.openTab(player, -4922); break;
		 */
		case 8654:
		case 8657:
		case 8655:
	...
	private boolean checkHandlers(Player player, int id) {
		...
		if (player.isPlayerLocked() && id != 2458 && id != -12760 && id != -12767 && id != -12780 && id != -12779
				&& id != -12778 && id != -29767 && id != -12763) {
			return true;
		}
