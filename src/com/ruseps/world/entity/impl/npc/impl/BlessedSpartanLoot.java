...
public class BlessedSpartanLoot extends NPC {
	...
	public static int[] MEDIUMLOOT = { 15247, 15248, 15249, 15253, 15254, 15255, 5079, 5080, 5081, 10682, 10683, 10684,
			10685, 915 };
	public static int[] RARELOOT = { 20650, 20651, 20652, 20653, 20654, 937, 18059, 109342 };
	public static int[] SUPERRARELOOT = { 938, 405, 18982, 18983, 19021, 18993, 2545, 10934, 13016, 18061 };
	...
	public static void giveLoot(Player player, NPC npc, Position pos) {
		...
		World.sendMessage("<img=469> <col=dbffba><shad=1>" + player.getUsername()
				+ " received a loot from the @gre@Blessed Spartan!");

		// GroundItemManager.spawnGroundItem(player, new GroundItem(new Item(19994,
		// Misc.inclusiveRandom(5, 10)), pos, player.getUsername(), false, 150, true,
		// 200));

		boolean isWearingCollector = DropUtils.hasCollItemEquipped(player);
		if (isWearingCollector) {
			player.addItemToAny(19990, Misc.inclusiveRandom(5, 10));
		} else
			GroundItemManager.spawnGroundItem(player, new GroundItem(new Item(19990, Misc.inclusiveRandom(5, 10)), pos,
	...
	public static final void loadDrops() {
		...
		items.put(19990, new NpcDropItem(19990, new int[] { 5 }, 0));

		Arrays.stream(COMMONLOOT).filter(i -> !items.containsKey(i))
				.forEach(i -> items.put(i, new NpcDropItem(i, new int[] { 1 }, 3)));
		Arrays.stream(MEDIUMLOOT).filter(i -> !items.containsKey(i))
				.forEach(i -> items.put(i, new NpcDropItem(i, new int[] { 1 }, 5)));
		Arrays.stream(RARELOOT).filter(i -> !items.containsKey(i))
				.forEach(i -> items.put(i, new NpcDropItem(i, new int[] { 1 }, 8)));
		Arrays.stream(SUPERRARELOOT).filter(i -> !items.containsKey(i))
				.forEach(i -> items.put(i, new NpcDropItem(i, new int[] { 1 }, 9)));
