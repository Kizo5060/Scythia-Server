...
public class Player extends Character {
    ...
    public PetBox getPetBox() { return petBox; }
    ...
    public void addDrBoost(int amount) {
        ...
        sendMessage("<img=0> You now have now added @red@" + getDrBoost() + " droprate!");
    }
    ...
    public int getAttackSpeed() {
        ...
        int weaponId47 = equipment.get(Equipment.WEAPON_SLOT).getId();
        if (weaponId47 == 11651) {
            speed = 0;
        }
    ...
    public CombatStrategy determineStrategy() {
        ...
        if (CustomMagicStaff.checkCustomStaff(this)) {
            ...
            this.setCastSpell(CustomMagicStaff.CustomStaff.getSpellForWeapon(this.getEquipment()
                    .getItems()[Equipment.WEAPON_SLOT].getId()));
            return CombatStrategies.getDefaultMagicStrategy();
        }

        if (castSpell != null || autocastSpell != null || this.getEquipment()
                .get(Equipment.WEAPON_SLOT)
                .getId() == 13058) {
            return CombatStrategies.getDefaultMagicStrategy();
        }
        ...
        if (getInventory().getFreeSlots() < (item.isNoted() ? 1 : item.isStackable() ? 1 : amount)) {
            for (int index = 0; index < 7; index++) {
                if (!getBank(index).isFull())
                    getBank(index).add(item.isNoted() ? item.getUnnotedId() : itemid, amount);
