...
public class PlayerUpdating {
    ...
    public static void update(final Player player) {
        ...
        for (Iterator<Player> playerIterator = player.getLocalPlayers()
                .iterator(); playerIterator.hasNext(); ) {
            Player otherPlayer = playerIterator.next();
            if (World.getPlayers()
                    .get(otherPlayer.getIndex()) != null && otherPlayer.isVisible() && !otherPlayer.isHidePlayer() && otherPlayer.getPosition()
                    .isWithinDistance(player.getPosition()) && !otherPlayer.isNeedsPlacement()) {
        ...
        for (Player otherPlayer : World.getPlayers()) {
            if (player.getLocalPlayers().size() >= 79 || playersAdded > MAX_NEW_PLAYERS_PER_CYCLE)
                break;
            if (otherPlayer == null || otherPlayer == player || !player.isVisible() || otherPlayer.isHidePlayer() || player.getLocalPlayers()
                    .contains(otherPlayer) || !otherPlayer.getPosition()
                    .isWithinDistance(player.getPosition()))
                continue;
	...
	/*	if (player.getCachedUpdateBlock() != null && !player.equals(target) && !updateAppearance && !noChat) {
        ...
        if (flag.flagged(Flag.CHAT) && !noChat && !player.getRelations()
                .getIgnoreList()
                .contains(target.getLongUsername())) {
            mask |= 0x80;
        }
    ...
    private static void updateGraphics(PacketBuilder builder, Player target) {
        ...
        builder.putInt(((target.getGraphic()
                .getHeight()
                .ordinal() * 50) << 16) + (target.getGraphic().getDelay() & 0xffff));
    }
    ...
    private static void updateAppearance(Player player, PacketBuilder out, Player target) {
        ...
        if (target.getNpcTransformationId() <= 0) {
            ...
            if (ItemDefinition.forId(equip[Equipment.BODY_SLOT]).isFullBody() || ItemDefinition.forId(target.getCosmeticEquipment()[Equipment.BODY_SLOT])
                    .getEquipmentType()
                    .equals(ItemDefinition.EquipmentType.PLATEBODY)) {
                properties.put(0);
            } else {
            ...
            ItemDefinition.EquipmentType originalHead = ItemDefinition.forId(equip[Equipment.HEAD_SLOT])
                    .getEquipmentType();
            ItemDefinition.EquipmentType cosmeticHead = ItemDefinition.forId(target.getCosmeticEquipment()[Equipment.HEAD_SLOT])
                    .getEquipmentType();
            ItemDefinition.EquipmentType fullHelmet = ItemDefinition.EquipmentType.FULL_HELMET;
            boolean isFullHelmet = cosmeticHead == fullHelmet || (originalHead == fullHelmet && cosmeticHead != ItemDefinition.EquipmentType.HAT && cosmeticHead != ItemDefinition.EquipmentType.FULL_MASK);
            if (isFullHelmet) {
                properties.put(0);
            } else {
            ...
            if (appearance.getLook()[Appearance.BEARD] <= 0 || appearance.getGender()
                    .equals(Gender.FEMALE) || ItemDefinition.forId(equip[Equipment.HEAD_SLOT])
                    .isFullHelm()) {
                properties.put(0);
            } else {
        ...
        Item firstCustom = target.getEquipment().getItems()[Equipment.BOOSTER];
        Item secondCustom = target.getEquipment().getItems()[Equipment.AURA];
        ...
        Arrays.stream(target.getCosmeticEquipment()).forEach(item -> properties.putShort(Math.max(item, 0)));
