...
public class GamblingManager extends PacketInteraction {
	...
	public enum GameType {
		 ...
		 * Dice dueling
		 */
		DICE_DUEL(30895, "@or2@Duel your partner in\\n" + "a dice gamble. There\\n" + "are 3 rounds. Beat\\n"
				+ "your opponent by\\n" + "rolling a higher\\n" + "number than them.", 3),
		/**
		 * 55x2
		 */
		FIFTY_FIVE(30893, "@gre@A very quick game where the dice is rolled. If the number\\n"
				+ "@gre@is lower than 55 then the opponent wins the game.", 1),
		/**
		 * Black jack
		 */
		BLACK_JACK(30896,"@or2@Duel your partner in\\n" + "a dice gamble. There\\n" + "are 3 rounds. Beat\\n"
				+ "your opponent by\\n" + "rolling a higher\\n" + "number than them.", 4),
		/**
		 * Frosty flowers
		 */
		 ...
		 * Flower poker
		 */
		FLOWER_POKER(30894, "@gre@Plant 5 flowers and use the rules of poker to play\\n"
				+ "@gre@and determine the\\n" + "@gre@winner of the game.\\n", 2),
		/**
		 * Abc flowers,
		 */
	...
	public static void sendInvitation(final Player player, Player otherPlayer) {
		...
		if(player.getGameMode() == GameMode.GROUP_IRONMAN
			...
			player.sendMessage("@red@Iron men can't gamble!");
			return;
		}
		...
		if (!Objects.isNull(requested)) {
			...
			if(!player.getInventory().contains(666)) {
				player.sendMessage("@red@ Sorry " + player.getName() + " you require a Gambling Pass to send Invites." );
				return;
			}
	...
	public static void openOfferScreen(final Player player, Player otherPlayer) {
		...
		player.getPacketSender().setScrollBar(28350, 0);
		player.getPacketSender().setScrollBar(28352, 0);
		/**
		 * Interface
		 */
	...
	private static void removeFlowers(Player player) 
	{
		for (GameObject gameFlower : player.getGambling().getGameFlowers()) 
		{
			 ...
			 CustomObjects.spawnGlobalObject(remove);
			 Player otherPlayer = player.getGambling().getRequested();
	...
	public boolean handleButtonInteraction(final Player player, final int button) {
		...
		switch (button) {
		case 30914:
			player.sendMessage("Clicked accept");
