...
import com.ruseps.world.content.minigames.Minigame3;
import com.ruseps.world.entity.impl.npc.NPC;
import com.ruseps.world.entity.impl.player.Player;
import com.ruseps.model.Locations.Location;

public abstract class Raid2 extends Minigame3 {
	protected Stage[] stages;
	protected int currentStage = -1;


	protected Position endPos = new Position(2801, 2619, 0);
	...
	public Raid2(int maxPlayers, Location location, Stage[] stages) {
	...
	public Raid2(int maxPlayers, Location location) {
	...
	protected void processActive() {
		...
		if (currentStage == -1) { // actual raid hasn't started yet
			startNextStage();
			MarvelsRaid.setStarted(true);
		}

		if (stages[currentStage].isFinished() && currentStage == stages.length - 1) {
			giveRewards();
			MarvelsRaid.setStarted(false);
			playersToEndPos();
			init();
			return;
		}
		if (playerIndex == 3) {
			World.sendMessage("@red@[Marvel Raids] <col=16777215>" + getName() + " ended with no winners");
			MarvelsRaid.setStarted(false);
			init();
			return;
		}
		if (playerIndex == 1 && !lastPlayerAnnouncement) {
			World.sendMessage("@red@[Marvel Raids] <col=16777215>" + players[0].getUsername() + " is left alone in " + getName());
	...
	public boolean handleNpcDeath(NPC npc) {
		...
		for (int i = 0; i < playerIndex; ++i) {
			...
			if(Misc.random(150) == 1) {
				players[i].getInventory().add(15501,1);
				World.sendMessage("<shad=0>@bla@[@mag@Marvels Box@bla@]@mag@ "+players[i].getUsername()+ "@bla@ has just received a @mag@Marvels Box!@bla@");
				
			}
			
			if(Misc.random(1000) == 1) {
					players[i].getInventory().add(18338,1);
