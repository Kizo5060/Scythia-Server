...
public class Trading {
	...
	public void requestTrade(Player player2) {
		...
		if (player.getGameMode() == GameMode.GROUP_IRONMAN && !player.isGim()) {
			player.getPacketSender().sendMessage("Group Ironman players are not allowed to trade.");
			return;
		}
		...
		if (player2.getGameMode() == GameMode.IRONMAN && !player.isGim()) { // ok fixed that part
			// as for the account creation thats up to u since idk how ur account creation interface works
			// but it seems like the button doesnt work at all
		...
		if (player.isGim() && !player.getGroupIronman().canTrade(player2)) {
			System.out.println("You can only trade your group members");
			return;
		}
