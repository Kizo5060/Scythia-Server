...
public class Slayer {
	...
	public void handleSlayerTaskDeath(boolean giveXp) {
		...
		if(amountToSlay > 1) {
            ...
            .progress(com.ruseps.world.content.achievements.AchievementData.COMPLETE_A_SLAYER_TASK, 1);
			if(slayerTask.getTaskMaster() == SlayerMaster.KURADEL) {
			player.getAchievementTracker()
            .progress(com.ruseps.world.content.achievements.AchievementData.COMPLETE_A_HARD_SLAYER_TASK, 1);
			} else if(slayerTask.getTaskMaster() == SlayerMaster.SUMONA) {
			player.getAchievementTracker()
	        .progress(com.ruseps.world.content.achievements.AchievementData.COMPLETE_AN_ELITE_SLAYER_TASK, 1);
			}
			lastTask = slayerTask;
	...
	public void givePoints(SlayerMaster master) {
		...
		if (Skillcape_Data.SLAYER.isWearingCape(player)) {
			pointsReceived += pointsReceived + 10;
		}
		int per5 = pointsReceived * 4;
		int per10 = pointsReceived * 6;

		if (GameLoader.getDay() == GameLoader.WEDNESDAY || GameLoader.getDay() == GameLoader.THURSDAY || GameLoader.getDay() == GameLoader.TUESDAY || GameLoader.getDay() == GameLoader.FRIDAY || GameLoader.getDay() == GameLoader.SATURDAY || GameLoader.getDay() == GameLoader.SUNDAY) {
			pointsReceived *= 2;
		}
