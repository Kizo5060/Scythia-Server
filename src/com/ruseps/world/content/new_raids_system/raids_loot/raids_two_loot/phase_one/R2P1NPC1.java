 ...
 *
 */
public class R2P1NPC1 extends NPC {
	 ...
	 * The npc id.
	 */
	public static final int FIRST_NPC_ID = 1500;
	
	public R2P1NPC1(Position position) {
	...
	public static void giveLoot(Player player, NPC npc, Position pos) {
		...
		if (chance > 99) {
			player.getPointsHandler().incrementRaidsTwoPoints(15 + (Misc.getRandom(5)));
			player.getPacketSender().sendMessage("<img=10>@blu@"+player.getUsername()+" you have received Raids two Points!");
			player.getPacketSender().sendMessage("<img=10>@blu@You now have " + player.getPointsHandler().getRaidsOnePoints() + " points.");
			return;
		}
		
		if (chance > 95) {
			player.getPointsHandler().incrementRaidsTwoPoints(10 + (Misc.getRandom(5)));
			player.getPacketSender().sendMessage("<img=10>@blu@"+player.getUsername()+" you have received Raids two Points!");
			player.getPacketSender().sendMessage("<img=10>@blu@You now have " + player.getPointsHandler().getRaidsOnePoints() + " points.");
			return;
		}
		...
		if (chance > 50) {
			player.getPointsHandler().incrementRaidsTwoPoints(5 + (Misc.getRandom(5)));
			player.getPacketSender().sendMessage("<img=10>@blu@"+player.getUsername()+" you have received Raids two Points!");
			player.getPacketSender().sendMessage("<img=10>@blu@You now have " + player.getPointsHandler().getRaidsOnePoints() + " points.");
			return;
		}
		...
		if (chance >= 0) {
			player.getPointsHandler().incrementRaidsTwoPoints(5);
			player.getPacketSender().sendMessage("<img=10>@blu@"+player.getUsername()+" you have received 5 Raids two Points!");
			return;
		} 
	}
