...
public class GoldBox {
	...
	public static int loot[][] = { { 2759, 2760, 2761, 2762, 2764, 10942 },
			{ 2765, 2766, 2767, 2773, 2775, 2777, 2779, 2781, 2784, 2787 }, { 2749, 2751, 2753, 2756, 2754, 2755 },
			{ 19082, 19080, 19084, 21023, 21024, 21025, 21026, 21027, 21028, 18060, 1002 },
			{ 21056, 19085, 18062, 18961 }, { 10935, 18689, 6854, 18960 } };
	...
	public void spin() {
		...
		if (!plr.getInventory().contains(BOX)) {
			plr.sendMessage("You require a Gold box to do this.");
			return;
		}
	...
	public void process() {
		...
		int[] common = { 2768, 2769, 2770, 2771, 2772, 2789 };
		int[] uncommon = { 2759, 2760, 2761, 2762, 2764, 10942 };
		int[] veryUncommon = { 2765, 2766, 2767, 2773, 2775, 2777, 2779, 2781, 2784, 2787 };
		int[] rare = { 2749, 2751, 2753, 2756, 2754, 2755 };
		int[] veryRare = { 19082, 19080, 19084, 21023, 21024, 21025, 21026, 21027, 21028, 18060, 1002 };
		int[] extremelyRare = { 21056, 19085, 18062, 18961 };
		int[] legendary = { 10935, 18689, 6854, 18960 };
		// 10$ 10942
		int[][] all = { common, uncommon, veryUncommon, rare, veryRare, extremelyRare, legendary };
		/*
		 * int[] tier = all[Misc.random(6)]; int PRIZE_ID = tier[Misc.random(tier.length
		 * -1)];
		 */
		...
		// final int PRIZE_ID = 1040; // TODO: Add box prize logic here, use
		// mysteryPrizeTier too (0-6) for different coloured reward text
	...
	public void rewardAll() {
		// plr.sendMessage("@red@Items will be add to your bank, If your inventory is
		// full.");
		// new Thread(() -> {
		...
		int[] rare = { 2749, 2751, 2753, 2756, 2754, 2755 };
		int[] veryRare = { 19082, 19080, 19084, 21023, 21024, 21025, 21026, 21027, 21028, 18060, 1002 };
		int[] extremelyRare = { 21056, 19085, 18066, 18961, 18062 };
		int[] legendary = { 10935, 18689, 6854, 18960 };
		int boxId = BOX;
		...
		// plr.getProgressionManager().getProgressionTask(13).incrementProgress(0,
		// amount);
		// plr.getDailyTaskManager().submitProgressToIdentifier(5, amount);
		// plr.getProgressionManager().getProgressionTask(16).incrementProgress(0,
		// amount);
		...
		/*
		 * int[][] all = {common, uncommon, veryUncommon, rare, veryRare, extremelyRare,
		 * legendary};
		 * 
		 * int[] tier;
		 * 
		 * int random = RandomUtility.exclusiveRandom(0, 100); // Generates a random
		 * number between 0 and 99 if (random < 40) { tier = all[0]; // Common (40%
		 * probability) } else if (random < 65) { tier = all[1]; // Uncommon (25%
		 * probability) } else if (random < 75) { tier = all[2]; // Very Uncommon (10%
		 * probability) } else if (random < 85) { tier = all[3]; // Rare (5%
		 * probability) } else if (random < 90) { tier = all[4]; // Very Rare (5%
		 * probability) } else if (random < 95) { tier = all[5]; // Extremely Rare (5%
		 * probability) } else { tier = all[6]; // Legendary (5% probability) }
		 * 
		 * int PRIZE_ID = tier[Misc.random(tier.length -1)];
		 * 
		 * //final int PRIZE_ID = 1040; // TODO: Add box prize logic here, use
		 * mysteryPrizeTier too (0-6) for different coloured reward text mysteryPrize =
		 * PRIZE_ID; // For rewards handling
		 * 
		 * plr.getInventory().delete(BOX); reward(); //plr.addItemToAny(PRIZE_ID, 1); }
		 * }).start();
		 */
		mysteryPrize = -1;
		canMysteryBox = true;
