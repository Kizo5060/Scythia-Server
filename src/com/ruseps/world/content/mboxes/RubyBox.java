...
public class RubyBox {
	...
	public static int loot[][] = {{ 13034, 8680, 8668, 17668, 10942 },
			{13033, 13032, 8672, 8674, 9077, 17666, 13035, 10537, 18059},
					{ 10934 ,10942, 17660, 8676},
				{ 21055 ,18689,18980 },
					{18066,13021, 21056},
				{10943, 18963,18957},
				{1413, 18981, 19005, 19016, 19004 }};

	/**
	 * Constructs a new mystery box to handle item receiving for this player alone
	 * @param plr the player
	 */
	public RubyBox(Player plr) {
		this.plr = plr;
	}
	...
	public void spin() {
		...
		if (!plr.getInventory().contains(BOX)) {
			plr.sendMessage("You require a Ruby box to do this.");
			return;
		}
	...
	public void process() {
		...
		int[] common = { 13034, 8680, 8668, 17668, 10942 };
		int[] uncommon = {13033, 13032, 8672, 8674, 9077, 17666, 13035, 10537, 18059};
		int[] veryUncommon = { 10934 ,10942, 17660, 8676};
		int[] rare = { 21055 ,18689,18980 };
		int[] veryRare = {18066,13021, 21056};
		int[] extremelyRare = {10943, 18963,18957,  };
		int[] legendary = {1413, 18981, 19005, 19016, 19004 };
 //10$ 10942 
		...
		int[] tier;
		   ...
		   int random = r.nextInt(101); // Generates a random number between 0 and 99
	        ...
	        if (random <= 40) {
	        	...
	        	tier = all[3]; // Rare (9% probability)
	        } else if (random <= 91) {
	        	tier = all[4]; // Very Rare (7% probability)
	        } else if (random <= 97) {
	        	tier = all[5]; // Extremely Rare 6% probability)
	        } else {
	        	tier = all[6]; // Legendary (3% probability)
	        }
		int PRIZE_ID = tier[Misc.random(tier.length -1)];
