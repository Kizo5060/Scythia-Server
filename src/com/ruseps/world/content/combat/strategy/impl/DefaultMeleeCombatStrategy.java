...
public class DefaultMeleeCombatStrategy implements CombatStrategy {
     ...
     */
    private void startAnimation(Character entity, Character victim) {
		if (entity.isNpc()) {
			NPC npc = (NPC) entity;
			...
			if (!player.isSpecialActivated()) {
				if(WeaponAnimations.getAttackGraphics(player) > 0) {
					victim.setGraphic(new Graphic(WeaponAnimations.getAttackGraphics(player)));
				}
