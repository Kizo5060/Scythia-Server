...
public class DragonStoneStrat implements CombatStrategy {

	boolean transformed = false;
	
	private void transform(NPC npc, int id) {
		npc.setTransformationId(id);
		npc.forceChat("THANKS FOR YOUR DONATIONS IT SETS ME FREE!!!");
	...
	public boolean customContainerAttack(Character entity, Character victim) {
		...
		//	vader.forceChat("THANKS FOR YOUR DONATIONS IT SETS ME FREE!!!");
			transformed = false;
		}
		...
		if(Locations.goodDistance(vader.getPosition().copy(), victim.getPosition().copy(), 1) && Misc.getRandom(5) <= 3) {
			...
			vader.performAnimation(new Animation(11991));
			vader.performGraphic(new Graphic (2009));
			vader.getCombatBuilder().setContainer(new CombatContainer(vader, victim, 1, 3, CombatType.MELEE, true));
				...
				public void execute() {
					if(tick == 0 && !barrage) {
						...
						if(barrage && victim.isPlayer() && Misc.getRandom(10) <= 5) {
							vader.forceChat("Stay WHERE You Are!");
							victim.getMovementQueue().freeze(3);
							victim.forceChat("What! I can't move!");
							victim.performGraphic(new Graphic(1177));
						}
						if(barrage && Misc.getRandom(6) <= 3) {
							vader.performAnimation(new Animation(2661));
							for(Player toAttack : Misc.getCombinedPlayerList((Player)victim)) {
								if(toAttack != null && Locations.goodDistance(vader.getPosition(), toAttack.getPosition(), 7) && toAttack.getConstitution() > 0) {
									new CombatHitTask(vader.getCombatBuilder(), new CombatContainer(vader, toAttack, 2, CombatType.MELEE, false)).handleAttack();
									toAttack.performGraphic(new Graphic(1308));
									//vader.forceChat("Goodnight Little BOIS!!!");
								}
							}
						}
