...
public class BlessedSpartan implements CombatStrategy {
	...
	public boolean customContainerAttack(Character entity, Character victim) {
		...
		if(Locations.goodDistance(vader.getPosition().copy(), victim.getPosition().copy(), 1) && Misc.getRandom(5) <= 3) {
			...
			vader.performAnimation(new Animation(4230));
			...
			vader.performAnimation(new Animation(barrage ? 4230 : 4230));
			...
			TaskManager.submit(new Task(1, vader, false) {
				...
				public void execute() {
					if(tick == 0 && !barrage) {
						new Projectile(vader, victim, 195, 44, 3, 43, 43, 0).sendProjectile();
					} else if(tick == 1) {
