...
public class DiamondDragon implements CombatStrategy {
	
	boolean transformed = false;
	
	private void transform(NPC npc, int id) {
		npc.setTransformationId(id);
		npc.forceChat("ARGGGGGGGGH!");
		npc.getUpdateFlag().flag(Flag.TRANSFORM);
	}
	
	private void transform2(NPC npc, int id) {
		npc.setTransformationId(id);
		npc.forceChat("Ever heard of Purple Rain!");
		npc.getUpdateFlag().flag(Flag.TRANSFORM);
	}
	
	private void transform3(NPC npc, int id) {
		npc.setTransformationId(id);
		npc.forceChat("Riveting Rubies!");
		npc.getUpdateFlag().flag(Flag.TRANSFORM);
	}
	...
	public boolean customContainerAttack(Character entity, Character victim) {
		...
		if (vader.getConstitution() < vader.getDefaultConstitution() / 3 && !transformed) {
			transform2(vader, 165);
			transformed = true;
		}
		if (vader.getConstitution() < vader.getDefaultConstitution() / 2 && !transformed) {
			transform3(vader, 175);
			transformed = true;
		}
