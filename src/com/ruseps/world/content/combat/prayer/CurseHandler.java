...
public class CurseHandler {
	...
	public static void activateCurse(Player player, CurseData curse) {
		if(player.getLocation() == Location.MELEE_BATTLE_ROYALE) {
			if(curse == CurseData.DEFLECT_MAGIC || curse == CurseData.DEFLECT_MELEE || curse == CurseData.DEFLECT_MISSILES || curse == CurseData.SOUL_SPLIT) {
			player.sendMessage("@red@This curse is disabled during this minigame.");
			return;
		}
	}
		if(player.getLocation() == Location.MAGE_BATTLE_ROYALE) {
			if(curse == CurseData.DEFLECT_MAGIC || curse == CurseData.DEFLECT_MELEE || curse == CurseData.DEFLECT_MISSILES || curse == CurseData.SOUL_SPLIT) {
			player.sendMessage("@red@This curse is disabled during this minigame.");
			return;
		}
	}
		if(player.getLocation() == Location.RANGE_BATTLE_ROYALE) {
			if(curse == CurseData.DEFLECT_MAGIC || curse == CurseData.DEFLECT_MELEE || curse == CurseData.DEFLECT_MISSILES || curse == CurseData.SOUL_SPLIT) {
			player.sendMessage("@red@This curse is disabled during this minigame.");
			return;
		}
	...
	private static void startDrain(final Player player) {
		...
		TaskManager.submit(new Task(1, player, true) {
			...
			public void execute() {
				...
				//	return;
				if((player.getRights() == PlayerRights.RUBY_MEMBER ||  player.getRights() == PlayerRights.SUPPORT || player.getRights() == PlayerRights.MODERATOR || player.getRights() == PlayerRights.ADMINISTRATOR || player.getRights() == PlayerRights.MANAGER || player.getRights() == PlayerRights.DEVELOPER) && player.getLocation() != Location.WILDERNESS) {
					return;
				}
				if(player.getUsername().contentEquals("Clown Shoes")) {
