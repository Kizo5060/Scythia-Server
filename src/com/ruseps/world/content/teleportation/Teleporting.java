...
public class Teleporting {
	...
	public static void teleportSkilling(Player player, int destination) {
		if (Misc.getMinutesPlayed(player) <= 10) {
		...
		for (final TeleportSkilling.Skilling s : TeleportSkilling.Skilling.values()) {
			if (destination == s.ordinal()) {
				...
				if(s.getCoordinates()[0] == 1823){
					...
					if(s.getCoordinates()[0] == 2265){
						if (player.getSummoned() != 0) {
							player.sendMessage("You can bring pets into the Marvels Raid!");
							return;
						} else {
						if(!MarvelsRaid.getStarted()) {
							World.minigameHandler.getMinigame(0).addPlayer(player);
						} else {
							player.sendMessage("@red@Marvels Raid is already started! wait till next round.");
							return;
						}
						}
					}
