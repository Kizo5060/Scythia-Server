package com.ruseps.world.content.diamond_coin_conversion;
...
public class DiamondCoinConversion {

    public static boolean onBank(Player player, int itemId) {
        if(itemId == DiamondCoinConstants.DIAMOND_COIN_ID) {

            long tokensToConvert = player.getInventory().getAmount(DiamondCoinConstants.DIAMOND_COIN_ID);

            long coinsInInventory = player.getInventory().getAmount(19994);

            long coinsToReceive = tokensToConvert * DiamondCoinConstants.PRICE_PER_TOKEN;

            long coinsPossible = coinsToReceive + coinsInInventory;

            if(coinsPossible > Integer.MAX_VALUE) {
                tokensToConvert = (Integer.MAX_VALUE - coinsInInventory) / DiamondCoinConstants.PRICE_PER_TOKEN;

                coinsToReceive = tokensToConvert * DiamondCoinConstants.PRICE_PER_TOKEN;
            }
                ...
                public Dialogue nextDialogue() {
                    return new Dialogue() {
                        ...
                        public boolean action(int option) {
                            ...
                            if(option == 1) {
                                if (player.getInventory().contains(new Item(DiamondCoinConstants.DIAMOND_COIN_ID, finalTokensToConvert))) {
                                    player.getInventory().delete(DiamondCoinConstants.DIAMOND_COIN_ID, finalTokensToConvert);
            ...
            int tokensInInventory = player.getInventory().getAmount(DiamondCoinConstants.DIAMOND_COIN_ID);

            int tokensToReceive = coinsToConvert / DiamondCoinConstants.PRICE_PER_TOKEN;
                        ...
                        public boolean action(int option) {
                            ...
                            if (option == 1) {
                                if (player.getInventory().contains(new Item(19994, finalCoinsToConvert))) {
                                    ...
                                    player.getInventory().add(DiamondCoinConstants.DIAMOND_COIN_ID, finalTokensToReceive);
